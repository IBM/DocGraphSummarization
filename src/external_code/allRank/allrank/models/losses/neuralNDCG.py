...
def neuralNDCG(y_pred, y_true, padded_value_indicator=PADDED_Y_VALUE, temperature=1., powered_relevancies=False, k=None,
    ...
    if k is None:
        ...
        k = int(y_true.shape[1] * k)
    else:
        k = k if k < y_true.shape[1] else y_true.shape[1]
    ...
    #discounted_gains = torch.nan_to_num(discounted_gains)

    if powered_relevancies:
        idcg = dcg(y_true, y_true, ats=[k]).permute(1, 0)
    else:
        idcg = dcg(y_true, y_true, ats=[k], gain_function=lambda x: x).permute(1, 0)
    #idcg = torch.nan_to_num(idcg)

    discounted_gains = discounted_gains[:, :, :k]
    # set idcg inf values to large number
    #large_number = 1*10**(10)
    #idcg[idcg == float("inf")] = large_number 
